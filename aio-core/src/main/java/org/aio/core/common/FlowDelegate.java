...
/**
 * Bidirectional flow delegate using two SubscriberWrappers.
 *
 * <p> Constructor takes two Flow.Subscribers: one that receive the IN data
 * (after it has been treated by FlowDelegate), and one that receive the OUT
 * data to send (before it has been treated by FlowDelegate).
 *
 * <p> Methods upstreamReader() and upstreamWriter() return the corresponding
 * Flow.Subscribers containing Flows for the treated upstream data.
 * See diagram below.
 *
...
//    final Consumer<ByteBuffer> recycler;
    ...
    /**
     * Creates an FlowDelegate fed from two Flow.Subscribers. Each
...
//        this.handshakeState = new AtomicInteger(NOT_HANDSHAKING);
     ...
     * for errors that need to be signaled from downstream to upstream.
     */
//    public FlowDelegate(SSLEngine engine,
//                        Executor exec,
...
//    }
    ...
    /**
     * Connects the read sink (downReader) to the FlowDelegate Reader,
     * and the write sink (downWriter) to the FlowDelegate Writer.
