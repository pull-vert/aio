 ...
 *
 * Register with {@link SelectableEndpoint#registerTimer(TimeoutEvent)}}.
 *
 * Cancel with {@link SelectableEndpoint#cancelTimer(TimeoutEvent)}.
 */
public abstract class TimeoutEvent implements Comparable<TimeoutEvent> {
    ...
    public int compareTo(TimeoutEvent other) {
        ...
        final var compareDeadline = this.deadline.compareTo(other.deadline);
        if (compareDeadline == 0 && !this.equals(other)) {
            var diff = this.id - other.id; // should take care of wrap around
            if (diff < 0) return -1;
